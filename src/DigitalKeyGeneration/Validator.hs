...
-- -- Redeemer type
-- data GuestRedeemer = Submit | GenerateAndValidateKey | CheckOut
...
--             traceIfFalse "Check-in flag not updated"      (initiateCheckIn outDatum == False) &&
...
--                   traceIfFalse "Check-in already initiated" (initiateCheckIn dat == False) &&
--                   traceIfFalse "Digital key already set"    (digitalKey dat == emptyByteString) &&

--                   traceIfFalse "Digital key not submitted"       (digitalKey outDatum /= emptyByteString) &&
--                   traceIfFalse "Digital key must be validated"   (isDigitalKeyValidated outDatum == True) &&
--                   traceIfFalse "Check-in flag must be set"       (initiateCheckIn outDatum == True) &&
